...
they called into the existing file server. User programs can be switched back to
...
journal queue BD in a hold state, so that all filesystem changes are queued up.
...
changes in the block that need to be evicted. These changes then point to other
changes, and so on.
