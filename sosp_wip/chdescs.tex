...
\begin{figure}
  ...
  \caption{\label{fig:journal} Journal change descriptor graph. Original FS data
  change descriptors ($A$, $B$, $C$, and $D$) depend on a journal commit record,
  which depends on copies of the original change descriptors ($A_j$, $B_j$,
  $C_j$, and $D_j$) being written to the journal. A commit record cancellation
  depends on the original change descriptors. The empty circles in the diagram
  are ``NOOP'' change descriptors which do not have associated block data.}
\end{figure}
