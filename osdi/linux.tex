...
\modules\ are wrappers for Linux's normal block devices, instead of the actual
device drivers that were in the original stand-alone KudOS version of the
\Kudos\ system. (In the FUSE version, they are wrappers for files in a standard
Linux file system.) These wrappers bypass the normal Linux buffer caches,
because \Kudos\ provides its own \chdesc-aware caches (\S\ref{sec:wbcache}) in
