...
We propose a new file system implementation architecture, called \emph{KFS},
...
change descriptors let KFS components preserve necessary file system invariants
without understanding the file system itself. Change descriptors can implement
many consistency mechanisms, including soft updates and journaling.

KFS is decomposed into fine-grained components which generate, consume, forward,
and manipulate change descriptors. A particular innovation of the component
design is the separation of the low-level specification of on-disk layout from
higher-level file system-independent code, which operates on abstract disk
structures. Our journaling module can automatically add journaling to any file
system, and combinations of simple components can support, for example, correct
consistency on RAID over loop-back devices.

Finally, a new interface between KFS and user applications allows them to have
some direct control over change descriptors. We have used this interface to
...
We have implemented KFS as both a Linux kernel module and a FUSE userspace file
system server. INSERT BRIEF PERFORMANCE MEASUREMENTS HERE.
