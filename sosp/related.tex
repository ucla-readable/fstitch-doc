...
journal to replay (or roll back) any operations that did not complete due to
the shutdown. A correct recovery procedure, though 
difficult to write~\cite{yang04using}, avoids time-consuming
\emph{fsck}-like file system
checks on reboot after a crash.  Sections~\ref{sec:patch:examples} and
\ref{sec:modules:journal} explain journaling with \patches.
