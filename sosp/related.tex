...
Dependency-like ideas have been used in
...
%
In particular, xsyncfs provides users with \emph{external synchrony}, which
...
before any external output by a process involved in that transaction, such
as terminal data or network traffic, is allowed to escape the kernel.
%
These dependencies are tracked across IPC as well.
%
\Featherstitch\ dependencies might be useful for representing xsyncfs
dependencies, or the cross-network dependencies send in BlueFS, more
generally; for instance, \Kudos\ patches apply at least conceptually to any
file system, whereas the xsyncfs implementation assumes a file system like
ext3.
%
Conversely, \Featherstitch\ applications could benefit from the the
...
zadok00fist,wright03ncryptfs,wright06versatility}, as does other extensions
to file system and disk interfaces~\cite{huang05fs2,sivathanu06typesafe};
these designs could potentially
benefit from a patchlike mechanism for representing write-before
