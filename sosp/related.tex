...
Section~4 of Burnett's thesis~\cite{burnett06information}. However, the
methods used to implement the interfaces are very different: Burnett's
system tracks dependencies at the level of entire system calls,
associates dirty blocks with unique IDs returned by those calls, and
duplicates dirty blocks when necessary to preserve ordering. Our work
tracks individual changes to blocks internally, allowing kernel modules
a finer level of control, and only chooses to expose an interface
similar to his to userspace as a means to simplify the sanity checking
which is required of arbitrary user-submitted requests.

Additionally, Burnett did not implement his system. Instead, it is
evaluated using trace-driven simulations. We have found that some
aspects of real disks have significant impacts which his simulator may
not have taken into account; for instance, the number of I/O requests to
the disk, rather than the number of blocks read or written by each
request, is generally the bottleneck in our tests.
