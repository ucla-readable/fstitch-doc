...
possible power failure or crashes via \emph{journaling}, which groups
...
%
Soft updates form in some ways a more challenging test of the patch
abstraction: their dependencies are more variable and harder to predict,
they are widely considered difficult to implement, and the existing FreeBSD
implementation is quite optimized~\cite{mckusick99soft}.
%
We therefore spend a fair amount of space discussing soft updates-like
dependency orders.
%
This should not be construed as a wholesale endorsement of soft updates,
which rely on a property (atomic block writes) that many disks do not
provide and which often require more seeks than journaling designs.
