...
a process are associated with that process's engaged \patchgroups\ and inherit
the appropriate dependencies, but are otherwise just like other changes --
they can be written as soon as their dependencies have been written.
%
\pgSync\ forces an immediate write of a \patchgroup\ to disk, and \pgClose\
disassociates a \patchgroup\ ID from the underlying \patches\ implementing it
(but otherwise does not affect those \patches).
%
...
\texttt{p} has \emph{ever} been engaged; if it succeeds, a subsequent
\texttt{\pgEngage(q)} will return an error.\footnote{These rules are really
