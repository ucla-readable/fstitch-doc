...
Patches are created with functions like \texttt{patch\_\-create\_\-byte}.
%
...
%
\Kudos\ therefore makes these patches depend on a ``commit'' empty \patch\
 that is explicitly held in memory.
%
Once the commit record is created, the empty patch is updated to depend on
 the actual commit record and then released.
%
Once the commit record is written, the empty patch is automatically
 ``written'' as well, which allows the main file system changes to follow.
%
We could explicitly represent empty \patches\ in our model by updating the
 in-flight safety property as follows:
%
...
On the one hand, empty patches can represent quadratic sets of dependencies
 ...
 all $n$ patches in $Q$, one could add $mn$ direct dependencies $p_i
 \PDDepend q_j$ or add an empty patch $x$ and $m+n$ direct dependencies
