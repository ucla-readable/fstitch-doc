...
\newcommand{\PDDepset}[1]{\ensuremath{\textit{DD}[#1]}}
...
Patch $p$'s block, written $\PBlock{p}$, is represented as a set containing
...
\qquad \quad Pick some block $B$ with $\PInf \cap B \neq \emptyset$. \\
\qquad \quad Commit and acknowledge each patch in $\PInf \cap B$.
\end{tabbing}
...
$\PDDepset{p}$ & $p$'s direct dependencies: other \patches\ that must be \\
	& committed before $p$ \\
\noalign{\vskip3pt}
...
%
Dependencies specify any ordering requirements among patches, and undo data
 allows a patch to be reverted in case its block must be written without
 it.
...
Every \patch\ in $\PDDepset{p}$ must be committed to disk either before
 $p$, or at the same time as $p$ (which is only possible if $p$ and $q$ are
 on the same block).
%
...
a file system wishing to impose strictly ordered writes might set
 $\PDDepset{p_n} = \{p_{n-1}\}$.
%
In practice, an upper file system layer defines an initial set of
 dependencies;
%
...
\caption{Example patch arrangements for an ext2-like file system.
 \textbf{a)} Example soft updates order for appending a zeroed-out block to
 a file.  \textbf{b)} A file on the same inode block is removed before the
 previous changes commit, inducing a circular block dependency.
 \textbf{c)} Example journal order for appending a zeroed-out block to a
 file.}
\label{f:ex}
\end{figure*}
...
$d\}$.
%
These \patches\ are depicted in Figure~\ref{f:ex}c.
%
...
%
Figure~\ref{f:ex}a depicts this arrangement.
...
Since the disk controller can write blocks in any order, \Kudos\ must ensure
