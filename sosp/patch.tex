...
We first fill in enough of our model to describe how disks behave, and
...
Our model's storage layer commits data to stable storage in units called
 \textbf{blocks}.
%
All writes affect one or more blocks, and it is impossible to selectively
 write part of a block.

Every change to a block's data is represented by a \textbf{patch}.
%
A patch applies to exactly one block, so a file system change that
...
The intermediate in-flight state models caching, reordering, and delay in
 ...
 committed by the disk controller in any order, possibly
...
Disk controllers act by writing in-flight patches one block at a
 time, which we model as follows:
%
\begin{tabbing}
...
Our model does not specify whether the storage system writes blocks
 atomically.
%
However, some file system designs, such as soft updates, rely on block
 write atomicity:
...
Every \patch\ in $\PDDepset{p}$ must be committed to disk either before
 ...
 on the same block).
%
The file system arranges dependencies corresponding to its desired consistency
 semantics.
%
