...
Given a block $\PB$, we write $\PDisk[\PB]$ for the set of committed
 patches with block $\PB$, or in notation $\PDisk[\PB] = \{p\in\PDisk\mid\PBlock{p}=\PB\}$.
%
...
\qquad \quad 2. Commit and acknowledge each patch in $\PInf[\PB]$.
\end{tabbing}
%
\noindent
%
...
%
soft updates expects that if the disk fails while a block $\PB$ is
 in flight, then $\PB$'s value on recovery must equal either the old value or
 the new value.
%
...
\end{comment}
%
Given this, $\PDepset{\PInf[\PB]} \cap \PDepset{\PInf[\PB']} \subseteq \PDisk$ for
 any $\PB' \neq \PB$, and the disk controller can write in-flight blocks
...
%
\begin{tabbing}
\quad \quad Pick some block $\PB$ with $\PMem[\PB] \neq \emptyset$ and $\PInf[\PB] =
\emptyset$. \\
\quad \quad Pick some $P \subseteq \PMem[\PB]$ with $\PDepset{P} \subseteq P \cup
\PDisk$. \\
...
\noindent
%
We require that $\PInf[\PB] = \emptyset$ to ensure that at most one version of
