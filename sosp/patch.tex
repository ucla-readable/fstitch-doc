...
\newcommand{\PDDepset}[1]{\ensuremath{#1.\textit{ddeps}}}
...
must be written to disk before $p$, or the file system's consistency is at
risk.
%
...
%
In summary, with the addition of dependencies, patches form a directed
graph; $\PDepset{p}$ is just the set of $p$'s successors in the graph.
...
%
That is, the dependencies of all patches on disk are also on disk.
%
Since the disk can write in-flight blocks in any order, a related
\textbf{in-flight safety property} must also hold: For any block $b$ with
$\PInf[b] \neq \emptyset$,
%
...
\begin{tabbing}
\quad \textbf{Write block.} \\
\qquad Pick some block $b$ with $\PMem[b] \neq \emptyset$. \\
\qquad Pick some $P \subseteq \PMem[b]$ with $\PDepset{P} \subseteq P \cup
\PDisk$. \\
\qquad For each $p \in P$, set $\PState{p} \gets \PInfst$. \\
\qquad For each $p \in \PMem[b]-P$, set $\PDDepset{p} \gets \PDDepset{p}
\cup P$.
\end{tabbing}
