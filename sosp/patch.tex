...
requires allocating a block by writing a bit to the block bitmap ($b_1$),
initializing the block by writing zeroes to it ($d_1$), writing the block number
to the inode for the file ($i_1$), and updating the file's size ($i_1'$).
%
By rule \ref{rule:pointer}, we need $i_1 \PDDepend b_1$ and $i_1 \PDDepend d_1$,
since $b_1$ and $d_1$ together initialize the new block.
%
By rule \ref{rule:reuse} we also need to make sure that $d_1$ depends on any
previous deallocation of the same block, so that it is not overwritten while
it is still in use.
%
A simple way to accomplish this is to let $d_1 \PDDepend b_1$.
%
Finally, we can let $i_1' \PDDepend d_1$ so that no file ever contains
