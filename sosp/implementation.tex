...
\Dodder\ buffer cache obeys all necessary dependency orderings.
%
A small kernel patch informs \Kudos\ of process fork and exit events as
required to update per-process \patchgroup\ state.


\Dodder\ modules interact with Linux's generic block layer mainly via
...
scheduler.
%
A callback notifies \Dodder\ when the disk controller reports request
...
NCQ and FUA, and a SATA2 Seagate drive (ST3320620AS).
