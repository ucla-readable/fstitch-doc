...
notifies \Kudos. As pointed out in \S\ref{sec:chdescs:model}, a request being
``complete'' might mean either that the data is merely in the drive's volatile
cache, or that it has been successfully stored on the physical media. The latter
situation, which is necessary for the strong guarantee \Kudos\ aims to provide,
can be arranged either by disabling the drive's write-back cache, or by using
the ``force unit access'' (FUA) mode when writing to the disk (though a small
patch to the Linux kernel is required to support this mode). Both of these
...
command queueing (TCQ) or SATA native command queueing (NCQ) only slightly slows
...
a required part of the SATA NCQ specification\todo{cite SATA spec}, so all SATA
disks support it when using NCQ.

Our interfaces bypass all Linux caches, because \Kudos\ provides its own
\chdesc-aware caches (\S\ref{sec:using:wbcache}) in order to provide its
