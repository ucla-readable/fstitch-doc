...
The first two types of merging involve hard patches.
%
The first type is trivial to explain:
%
...
%
A graph traversal prunes these cyclic
...
%
Once $p_h$ commits, the \patches\ in memory reach the state in
Figure~\ref{f:soft2hard}b.
%
$p_h$ is now irrelevant for cycle detection,
...
Consider a new \nrb\ \patch\ $p_h$ added to a block that contains some
\rb\ \patch\ $p$.
%
Since $p_h$ is considered to overlap $p$, \Kudos\ adds a direct dependency
$p_h \PDDepend p$.
%
