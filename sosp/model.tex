...
Let \depends{\p{p}}{\p{q}} for all \inset{q}{Q} where $Q \subseteq$ \ChAll %\\
...
Let \p{p}.state $:=$ \stateinf\ for all \inset{p}{P}

\paragraph{Disk writes block to media}
For some block $B$: \\
Let \p{p}.state $:=$ \statedisk\ for all \inset{p}{\ChInfB{B}}
...
%
\cdsafety{disk}{Disk \Befores}{\BeforeS{\ChDisk} \subseteq\ \ChDisk}
%
\noindent In essence, the system honors the
dependency information correctly. A second, and related, safety property:
%
\cdsafety{inflight}{In Flight \Befores}{\BeforeS{\ChInfB{\block{1}}} \cap \ChInfB{\block{2}} = \emptyset\ \mbox{for all}\ \block{1} \neq \block{2}}
%
