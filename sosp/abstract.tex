...
%
A basic building block of storage system reliability is the ``write before''
...
A journaled file system, for example, must write and commit each
 journal transaction before applying that transaction's changes.
...
These generalized file system dependencies are naturally exportable to
 user level, allowing applications to specify consistency
 protocols for the storage system to follow.
%
Our patch-based storage system, 
We describe a number of important
 optimizations for \patch-based storage systems and present \emph{\Kudos},
 a Linux kernel file system layer based on \patches.
%
The \Kudos\ ext2 prototype is competitive with Linux ext2 and ext3 and allows
...
%
Finally, we demonstrate three applications that use
 \patches\ to provide strong consistency guarantees without
 micromanaging writes to the disk.
