...
Talk about overlap merging. It's fast. It's simple. It's fun.
Who are the big users? Inode size updates. Bitmap blocks. Others?
...
To quickly determine whether a new \chdesc\ may need to be rolledback


Restricting the addition of befores to \chdescs\ being created and to
\noops\ with no afters\todo{Talk about this more? Elsewhere?} allows
strong reasoning about a \chdesc{}'s future.  Significantly, a
\chdesc\ created on a block for which there were no external afters on
any existing \chdesc\ on the block will \textit{never} need to be
rolled back. One can thus omit such a \chdesc{}'s rollback
information, creating a \textit{nonrollbackable} \chdesc.
