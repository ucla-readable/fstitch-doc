...
struct patch {
    ...
    struct patch_queue *dependencies;
...
patch_t *\textbf{patch_create_bit}(
    bdesc_t *block, BD_t *owner,
    uint16_t offset, uint32_t xor);
patch_t *\textbf{patch_create_noop}(
    BD_t *owner, patch_t *tail, ...);
int \textbf{patch_create_diff}(
    bdesc_t *block, BD_t *owner,
    uint16_t offset, uint16_t length,
    const void *newdata, patch_t **head);
int \textbf{patch_add_depend}(
    patch_t *depender, patch_t *dependee);
...
indicate that the \chdescs\ are considered to still have the above level. Afters
will thus remain at the above level (or higher), preserving the \chdescs's
dependencies. A \chdesc\ is \emph{satisfied} when its associated data reaches
stable storage.
...
at all. \Noop\ \chdescs\ can have dependencies, like any other
\chdesc, but they don't need to be written to disk:  they are trivially
satisfied when all of their dependencies are satisfied.
%
...
Depending on the arrangement of other change descriptors, \Kudos\ may have to
perform additional rollbacks and write some blocks more times than would
otherwise be necessary in order to write these \chdescs\ to disk. Even if that
is not necessary, \Kudos\ will still have less flexibility in choosing an order
to write the blocks, potentially increasing disk seek times due to suboptimal
ordering.
