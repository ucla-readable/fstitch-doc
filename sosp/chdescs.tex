...
\chdescinvar{Each block has a list of \chdescs\ on that block.}

\chdescinvar{Each block has a list of \chdescs\ on that block that are ready
  to be moved down by one level.}

\chdescinvar{Each block has a list of \chdescs\ that are not entirely overlaped.}

\chdescinvar{Each block tracks the total number of external afters for
  \chdescs\ on that block.}

\chdescinvar{Each block has at most one nonrollbackable \chdesc.}
